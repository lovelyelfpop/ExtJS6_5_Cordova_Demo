...
Ext.define('Ext.calendar.AbstractList', {extend:'Ext.dataview.DataView', onChildTap:function(context) {
...
edgePicker:{userCls:Ext.baseCSSPrefix + 'calendar-picker-list'}, applyPicker:function(picker, oldPicker) {
  ...
  var me = this, iconEl = me.iconEl, record;
  me.callParent([value, oldValue]);
  if (!iconEl) {
    me.iconEl = iconEl = me.bodyElement.createChild({cls:Ext.baseCSSPrefix + 'calendar-picker-field-icon'});
  }
...
}, privates:{queryTabletPicker:function(picker) {
  return picker.down('calendar-list');
}}});
Ext.define('Ext.calendar.form.TimeField', {extend:'Ext.field.Select', xtype:'calendar-timefield', config:{increment:15, format:'g:i A', minValue:null, maxValue:null}, initialize:function() {
  this.callParent();
  this.refreshOptions();
  ...
  var record = null, h, m, range, len, i, item, d;
  if (value) {
    if (value.isModel) {
  ...
  return value;
}, privates:{initDate:new Date(2008, 0, 1), refreshOptions:function() {
  var me = this, D = Ext.Date, min = me.getMinValue(), max = me.getMaxValue(), increment = me.getIncrement(), format = me.getFormat(), current = D.clone(this.initDate), options = [], end;
  if (max) {
    end = D.clone(current);
  ...
  while (current < end) {
    options.push({value:current, text:D.format(current, format)});
    current = D.add(current, D.MINUTE, increment);
  }
  me.setOptions(options);
}}});
Ext.define('Ext.calendar.form.AbstractForm', {extend:'Ext.form.Panel', requires:['Ext.calendar.form.CalendarPicker', 'Ext.calendar.form.TimeField', 'Ext.field.Text', 'Ext.field.TextArea', 'Ext.field.DatePicker', 'Ext.form.FieldSet', 'Ext.layout.HBox'], trackResetOnLoad:true, defaultListenerScope:true, platformConfig:{'!desktop':{width:'100%', height:'100%', layout:'fit', isCompact:true, fullscreen:true}, 'desktop':{modal:true, centered:true, scrollable:'y'}}, floated:true, config:{calendarField:{xtype:'calendar-calendar-picker', 
label:'Calendar', name:'calendarId', displayField:'title', valueField:'id'}, titleField:{xtype:'textfield', label:'Title', name:'title'}, startDateField:{xtype:'datepickerfield', label:'From', itemId:'startDate', name:'startDate'}, startTimeField:{xtype:'calendar-timefield', label:'\x26#160;', itemId:'startTime', name:'startTime'}, endDateField:{xtype:'datepickerfield', label:'To', itemId:'endDate', name:'endDate'}, endTimeField:{xtype:'calendar-timefield', label:'\x26#160;', itemId:'endTime', name:'endTime'}, 
allDayField:{xtype:'checkboxfield', itemId:'allDay', name:'allDay', label:'All Day', listeners:{change:'onAllDayChange'}}, descriptionField:{xtype:'textareafield', label:'Description', name:'description', flex:1, minHeight:'6em'}, dropButton:{text:'Delete', handler:'onDropTap'}, saveButton:{text:'Save', handler:'onSaveTap'}, cancelButton:{text:'Cancel', handler:'onCancelTap'}}, initialize:function() {
  var me = this;
  me.initForm();
  me.add({xtype:'toolbar', docked:'bottom', items:me.generateButtons()});
  me.callParent();
  me.applyValues();
  me.checkFields();
  ...
  buttons.push({xtype:'component', flex:1}, this.getCancelButton(), this.getSaveButton());
  return buttons;
}, fieldQuery:function() {
  ...
  var me = this, calField = me.getCalendarField();
  if (!calField.store) {
    calField.store = me.getCalendarStore();
  }
  me.add([{xtype:'fieldset', scrollable:me.isCompact ? 'y' : undefined, margin:0, layout:{type:'vbox', align:'stretch'}, items:[calField, me.getTitleField(), me.getStartDateField(), me.getStartTimeField(), me.getEndDateField(), me.getEndTimeField(), me.getAllDayField(), me.getDescriptionField()]}]);
...
Ext.define('Ext.calendar.panel.AbstractBase', {extend:'Ext.Panel', requires:['Ext.layout.Fit'], layout:'fit', autoSize:false, initialize:function() {
  var me = this;
  me.callParent();
  ...
  this.add(view);
}, privates:{handleRefresh:function() {
  this.handleResize();
...
Ext.define('Ext.overrides.calendar.view.Weeks', {override:'Ext.calendar.view.Weeks', doDestroy:function() {
  this.tip = Ext.destroy(this.tip);
  this.callParent();
}, privates:{hideOverflowPopup:function() {
  var tip = this.tip;
  if (tip) {
    tip.hide();
    tip.removeAll();
  ...
  if (!tip) {
    me.tip = tip = new Ext.tip.ToolTip({anchor:true, autoHide:false, ui:'calendar-overflow', cls:me.$overflowPopupCls, minWidth:200, border:true});
...
centered:false, enter:'left', exit:'left', hideOnMaskTap:true, stretchY:true, ui:'light', header:{border:false, title:'Calendars'}}, titleBar:{docked:'top'}}, items:[{xtype:'panel', reference:'mainContainer', flex:1, layout:'fit'}], autoSize:false, initialize:function() {
  ...
  var me = this, sheet = me.lookup('sheet'), vis;
  if (!me.isConfiguring) {
  ...
  var me = this, cfg = me.getCompact() ? me.createCompactTitleBar() : me.createNormalTitleBar();
  if (cfg) {
    me.lookup('mainContainer').add(cfg);
  }
}, createCompactTitleBar:function() {
  var me = this;
  return me.createTitleBar([Ext.apply({scope:me, handler:'onMenuButtonTap'}, me.getMenuButton()), me.createDateTitle({listeners:{element:'element', scope:me, tap:'onTodayTap'}}), {xtype:'component', flex:1}, me.createCreateButton()]);
  ...
  return this.createContainerWithChildren({reference:'titleBar'}, cfg, items);
}, createSheet:function() {
  return Ext.apply({layout:{type:'vbox', align:'stretch'}, items:[this.createCalendarList(), this.createSwitcher({vertical:true})]}, this.getSheet());
}, createSideBar:function() {
  var me = this, cfg = this.getSideBar(), items = [];
  if (!cfg) {
    return null;
  }
  if (me.getCreateButtonPosition() === 'sideBar') {
    items.push({xtype:'container', margin:'0 0 10 0', autoSize:null, layout:{type:'hbox', pack:'center'}, items:me.createCreateButton()});
  }
  items.push(me.createCalendarList());
  if (me.getSwitcherPosition() === 'sideBar') {
    items.push(me.createSwitcher({vertical:true}));
  }
  return this.createContainerWithChildren({reference:'sideBar', layout:'vbox'}, cfg, items);
  ...
  var me = this, sheet = me.lookup('sheet');
  if (!sheet) {
