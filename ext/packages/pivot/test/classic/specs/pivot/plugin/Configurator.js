...
function() {
    ...
    describe('configurator fields', function(){
        ...
        it('should show the field config menu when clicking on the field', function () {
            var spy = jasmine.createSpy();

            makeGrid(null, {
                listeners: {
                    showconfigfieldmenu: spy
                }
            });
            ...
            runs(function () {
                var panel = plugin.getConfigPanel(),
                    fieldsCt = panel.getAllFieldsContainer(),
                    column = fieldsCt.items.items[0],
                    childEls = ['textCol', 'sortCol', 'filterCol', 'customCol'],
                    len = childEls.length,
                    i;
                ...
                for (i = 0; i < len; i++) {
                    ...
                    // each child el should trigger the handler when clicked
                    jasmine.fireMouseEvent(column[childEls[i]], 'click');
